...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i < length; i++){
            for (int j = 0; j < length; j++){
                int currValue = board.tile(i, j).value();

                if (i == length - 1 && j == length - 1){
                    continue;
                }else if (i == length - 1){
                    int upValue = board.tile(i, j + 1).value();
                    if (currValue == upValue){
                        return true;
                    }
                }else if (j == length - 1){
                    int rightValue = board.tile(i + 1, j).value();
                    ...
                    int upValue = board.tile(i, j + 1).value();
                    int rightValue = board.tile(i + 1, j).value();
                    if (currValue == upValue || currValue == rightValue){
                        return true;
                    }
                }
            }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int length = board.size();
        Tile adjacentColomnTile = null;
        int adjacentColumnTileY = 0;

        for (int i = y + 1; i < length; i++){
            if (board.tile(x, i) != null){
                adjacentColomnTile = board.tile(x, i);
                adjacentColumnTileY = i;
                break;
            }
        }

        if (adjacentColomnTile == null){
            if ((length - 1) == y){
                return;
            }
        ...
        if (adjacentColomnTile.value() == myValue && !adjacentColomnTile.wasMerged()){
            targetY = adjacentColumnTileY;

            this.score += 2 * myValue;

        }else{
            targetY = adjacentColumnTileY - 1;
        }
